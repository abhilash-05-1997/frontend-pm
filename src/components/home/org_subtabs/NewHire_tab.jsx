...
const NewHire_tab = () => {
  ...
  const fetchNewHires = async () => {
    try {
      ...
      const response = await apiService.fetchInstance(
        `${GET_NEW_HIRES_URL}?filter_type=${filter}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiService.createInstance("accounts/register/", {
  ...
  return (
    ...
    //     className="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition duration-200 dark:bg-dark-add-button dark:font-bold dark:text-black left-0"
    //   >
    //     Add New Hire
    //   </button>
    //   <h1 className="text-2xl font-semibold text-start dark:text-dark-text">New Hires</h1>
    ...
    <div className="flex flex-col space-y-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2">
          <label htmlFor="filter" className="text-sm dark:text-dark-text">
            Filter by:
          </label>
          <select
            id="filter"
